	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				...
				FlxG.sound.playMusic(Paths.music('introduce'), 0.7);
		...
		robot = new FlxSprite(409, -69);
		robot.frames = Paths.getSparrowAtlas('title/robot');
		robot.animation.addByPrefix('idle', 'robot', 24, true);

		//if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(558.15, -69);
			gfDance.frames = Paths.getSparrowAtlas('title/waldoDanceTitle');
			gfDance.animation.addByPrefix('idle', 'WARDO', 24, true);
		//}
		...
		vig = new FlxSprite(-66, -75);
		vig.loadGraphic(Paths.image('title/vignetting'));

		robot.antialiasing = ClientPrefs.globalAntialiasing;
		add(robot);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(robot, {y:-1000}, 1.7, {ease: FlxEase.quartIn});
				FlxTween.tween(gfDance, {y:-1300}, 1.5, {ease: FlxEase.quartIn});
				FlxTween.tween(logoBl, {y:-700}, 1.5, {ease: FlxEase.quartIn});
				FlxTween.tween(titleText, {y:-500}, 1.5, {ease: FlxEase.quartIn});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				// credTextShit.screenCenter();
				case 7://5
					createCoolText(['Human'], 45);
				case 8://6
					addMoreText('MeotSi', 45);
				case 9://7
					addMoreText('RUDY', 45);
					addMoreText('K Pio', 45);
				...
				// credTextShit.screenCenter();
				case 15://13
					addMoreText('Stand up');
				// credTextShit.visible = true;
				case 16://14
					addMoreText('To');
				// credTextShit.text += '\nNight';
				case 17://15
					addMoreText('Dr.Waldo'); // credTextShit.text += '\nFunkin';
				case 18:
					deleteCoolText();
				case 19://16
					skipIntro();
