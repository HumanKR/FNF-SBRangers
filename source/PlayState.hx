...
class PlayState extends MusicBeatState
{
	...
	var rgbTimer:FlxTimer;
	var effects:Array<BitmapFilter> = [];
	public var rgbSplited:Bool = false;
	/*var multiplyBlend:BlendModeShader = new MultiplyShader();
	//var blends:Map<String, BlendModeShader> = ["Multiply" => new MultiplyShader()];
	var darkColor:FlxColor;
	var effectSans:BlendModeEffect;*/
	var shakeTrigger:Bool = true;
	var drainTrigger:Bool = true;
	...
	var diaArm:FlxSprite;
	var diaArm2:FlxSprite;
	...
	override public function create()
	{
		...
		effectSans = new BlendModeEffect(multiplyBlend, darkColor);
		FlxG.camera.setFilters([new ShaderFilter(cast effectSans.shader)]);*/
		...
		bpm2sec = (60000 / SONG.bpm) / 1000;
		trace('bpm2sec ' + bpm2sec);
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'kickstart':
					curStage = 'waldoStage';
				case 'warm-ups':
					curStage = 'waldoSecondStage';
				case 'overdrive':
					curStage = 'waldoFinalStage';
		...
		switch (curStage)
		{
			case 'waldoStage':
				...
				GameOverSubstate.endSoundName = 'gameOverEnd-azi';
				GameOverSubstate.characterName = 'azi';

				var bg:BGSprite = new BGSprite('waldo/background/bg', -1676, -1041, 1, 1);
				bg.updateHitbox();	
				add(bg);
				...
				robot = new BGSprite('waldo/character/bronze', -1543.1, -779.5, 1, 1, ['bronze'], true);
				robot.updateHitbox();
				add(robot);

				robot2 = new BGSprite('waldo/character/silver', -1547.2, -633.95, 1, 1, ['silver'], true);
				robot2.updateHitbox();
				add(robot2);
			...
			case 'waldoSecondStage':
				...
				var sky:BGSprite = new BGSprite('waldo/background/bgsky', 406.55, -1041.5, 1, 1);
				sky.updateHitbox();
				add(sky);
				...
				var robot:BGSprite = new BGSprite('waldo/character/gold', -1555.45, -633.2, 1, 1, ['gold'], true);
				robot.updateHitbox();
				add(robot);
			...
			case 'waldoFinalStage':
				GameOverSubstate.deathSoundName = 'azi_loss_sfx';
				GameOverSubstate.endSoundName = 'gameOverEnd-azi';
				GameOverSubstate.characterName = 'azilight';
				
				var bg:BGSprite = new BGSprite('waldo/background/bgthird', -1512, -1069, 1, 1);
				bg.updateHitbox();	
				add(bg);
				...
				diaArm.animation.addByPrefix('ready', 'diamond attack1', 24, false);
				diaArm.animation.addByPrefix('go', 'diamond attack2', 24, false);
				diaArm.updateHitbox();
				add(diaArm);

				diaArm2 = new FlxSprite(-1159.95, 149.6);
				diaArm2.frames = Paths.getSparrowAtlas('waldo/character/arm2');
				diaArm2.animation.addByPrefix('ready', 'diamond attack straight1', 24, false);
				diaArm2.animation.addByPrefix('go', 'diamond attack straight2', 24, false);
		...
		GameOverSubstate.camZoom = stageData.defaultZoom;

		if(isPixelStage) {
			introSoundsSuffix = '-pixel';
		}
		if(curStage != 'waldoStage')
			add(gfGroup);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'kickstart' | 'warm-ups' | 'overdrive':
					startDialogue(dialogueJson);

				/*case 'warm-ups': //just copy and paste cuz i'm lazy
					camHUD.visible = false;
					inCutscene = true;

					snapCamFollowToPos(1280, -285.3);
					FlxG.camera.focusOn(camFollow);
					...
					});*/ //our team hate this nooooooooooooo
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
					...
					else if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('growl')) {
						...
						if (curStage == 'waldoStage')
						{
							var grrRand = FlxG.random.int(0, 100);
							if (grrRand > 95)
							{
								var selectRand = FlxG.random.int(0, 1);
								if (selectRand == 0)
									gf.playAnim('grr', true, false, 12);
								else
									gf.playAnim('grr2', true);
							}
						}
					}

					if (curStage == 'waldoFinalStage')	
					{	
						if (shakeTrigger == true){	
							var shakeRand = FlxG.random.float(0.01, 0.05);	
							camGame.shake(shakeRand, 0.05, null, true, Y);	
						}	
						if (drainTrigger == true){	
							if (health > 0)	
								health -= 0.04;
						}
					}
		...
		if (!inCutscene) {
			if(!cpuControlled) {
				keyShit();
			} else if(boyfriend.holdTimer > Conductor.stepCrochet * 0.001 * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && !isDodging) {
				boyfriend.dance();
			}
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Trigger Trail': //fuk i give up this is suck
				var triggerTrail = Std.parseInt(value1);
				var triggerCharacter = Std.parseInt(value2);
				var targetTrail:Character;

				if(Math.isNaN(triggerTrail)) triggerTrail = 0;
				if(Math.isNaN(triggerCharacter)) triggerCharacter = 0;

				if (triggerCharacter == 0){
					targetTrail = boyfriend;
					remove(boyfriend);
				}
				else{
					targetTrail = dad;
				...
				var p2Trail = new FlxTrail(targetTrail, null, 4, 24, 0.6, 0.9);
				add(p2Trail);
				p2Trail.visible = false;
			...
			case 'Trigger dad Things':
				var shakeBool = Std.parseInt(value1);
				var drainBool = Std.parseInt(value2);
				//thats all haha
				if (shakeBool == 1) shakeTrigger = true;
				else shakeTrigger = false;
				if (drainBool == 0)
					drainTrigger = false;
				else
					drainTrigger = true;	
				//of course not heh....
				trace('drain is ' + drainTrigger);

			case 'Diamond Attack':
			{
				var readyTrig:Bool; 
				var attackNum:Int = Std.parseInt(value2);
				...
				if (attackNum == 1)
				{
					if (readyTrig)
					{
						diaArm.animation.play('ready');
						//FlxG.sound.play(Paths.sound('intro1'), 1);
						goSprite('1');
						FlxG.sound.play(Paths.sound('dia_click1'), 0.5);
					}
					else
					{
						diaArm.animation.play('go');
						FlxG.sound.play(Paths.sound('dia_clap'), 0.5);

						if (cpuControlled)	{ //ugh
						...
						//animation its 2 frame so... 2/24 = 0.08333.... oof
				...
				else
				{
					if (readyTrig)
					{
						...
						goSprite('Go');
						new FlxTimer().start(bpm2sec / 2, function(tmr:FlxTimer)
						{
							diaArm.animation.play('ready');
							FlxG.sound.play(Paths.sound('dia_click1'), 0.5);
							new FlxTimer().start(bpm2sec / 2, function(tmr:FlxTimer)
							{
								diaArm2.animation.play('ready');
								FlxG.sound.play(Paths.sound('dia_click2'), 0.5);
							});
						});
					}
					else
			...
			case 'Play Count Sound':
			{
				goSprite(Std.string(value1.toLowerCase()));
				//FlxG.sound.play(Paths.sound('count' + Std.string(value1)), 1);	
			}
		}
	...
	function goSprite(numThings:String) {
		//var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image('go'));
		//go.scrollFactor.set();
		var countSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(numThings.toLowerCase()));
		...
		countSpr.antialiasing = true;
		add(countSpr);
		FlxG.sound.play(Paths.sound('count' + numThings), 1);
		countDownSprites.push(countSpr);
		FlxTween.tween(countSpr, {y: countSpr.y += 100, alpha: 0}, Conductor.crochet / 1000, {
			ease: FlxEase.cubeInOut,
			onComplete: function(twn:FlxTween)
			{
				countDownSprites.remove(countSpr);
				remove(countSpr);
				countSpr.destroy();
			}
		});
	...
	function dodgeThing(?coolBreak:Bool = false):Void
	{
		if (coolBreak)
			canDodge = true;
			isDodging = false;
		if(FlxG.keys.justPressed.SPACE && !coolBreak) //aaa i used 'if' again... noooo
		{
			//trace('cheer up azi!');
			boyfriend.playAnim('dodge', true);
			canDodge = false;
			...
			new FlxTimer().start(0.25, function(tmr:FlxTimer) //i... just caculate with frame to second
			{
				isDodging = false;
				boyfriend.dance();
		...
		if (!note.wasGoodHit)
		{
			...
			if (curStage == 'waldoFinalStage'){	
				if (CoolUtil.difficultyString() == 'HARD')
				{
					if (note.noteData >= 0)	
						//health += 0.06;
						health += note.hitHealth + 0.025;	
					else	
						//health += 0.01;	
						health += note.hitHealth - 0.019;
				}
				else
					if (note.noteData >= 0)	
						//health += 0.06;
						health += note.hitHealth + 0.037;	
					else	
						//health += 0.01;	
						health += note.hitHealth - 0.013;
			}	
			else	
			{	
				if (note.noteData >= 0)	
	...
	public function resetThings(?tmr:FlxTimer):Void
	{
		if (!paused)
		{
			setRgb(0.0, 0.0);
	...
	public function rgbSplitEffect(p2:Bool):Void
	{
		if (curStage == 'waldoFinalStage' && ClientPrefs.flashing)
		{
			if (rgbTimer != null)
			{
				rgbTimer.cancel();
			...
			rgbTimer = new FlxTimer().start(2, resetThings, 1);
			var randomOffsetX1 = (FlxG.random.float(-3, -1) / 1000);
			var randomOffsetX2 = (FlxG.random.float(1, 3) / 1000);
			var randomOffsetY1 = (FlxG.random.float(-3, -1) / 1000);
			var randomOffsetY2 = (FlxG.random.float(1, 3) / 1000);

			var randomBoolX = FlxG.random.bool();
			var randomBoolY = FlxG.random.bool();
			var randomRGBX:Float;
			var randomRGBY:Float;

			if (randomBoolX)
				randomRGBX = randomOffsetX1;
			else
				randomRGBX = randomOffsetX2;

			if (randomBoolY)
				randomRGBY = randomOffsetY1;
			else
				randomRGBY = randomOffsetY2;

			//var randomOffset1 = 3 / 1000;
			//var randomOffset2 = 3 / 1000;
			if (p2)
			{
				randomRGBX *= 2;
				randomRGBY *= 2;
			...
			if (!rgbSplited)
			{
				trace('random1: ' + randomRGBX);
				trace('random2: ' + randomRGBY);
				setRgb(randomRGBX, randomRGBY);

				/*new FlxTimer().start(0.01, function(tmr:FlxTimer)
				{
					if (rgbXthing < 0)	rgbXthing = 0;
					setRgb(randomOffset1 - 0.001, randomOffset2 - 0.001);
					trace('random1: ' + randomOffset1);
					trace('random2: ' + randomOffset2);
				});*/
			}
			else
				setRgb(0.0, 0.0);
			rgbSplited = !rgbSplited;
		}
	}
	...
	override function beatHit()
	{
		...
		if(curBeat % 2 == 0) {
			if (boyfriend.animation.curAnim.name != null && !boyfriend.animation.curAnim.name.startsWith("sing") && !isDodging)
			{
				boyfriend.dance();
			}
